...
MCLoopBlock* MCLoopBlock::UnrollLoop(int64_t loops) {
  if (Type == Sequential) {
    MCLoopBlock *MB = new MCLoopBlock(Sequential, Start,0,0,this->getMode(), this->getEd());
    while (loops) {
      if (loops > 0) {
    ...
    if (loops == 0) return NULL;
      ...
      Count = Count + loops;
      if (-loops == 1)
        return new MCLoopBlock(Sequential, Start + Insts.size(), 0, 0, Insts, NULL, this->getMode(), this->getEd());
      return new MCLoopBlock(Repeat, Start + Insts.size(), -loops, 0, Insts, NULL, this->getMode(), this->getEd());
    }
    ...
    if (loops == 0) return this;
      ...
      if (!KINested) {
        while (((uint64_t) loops) >= abssize()) {
          if (!Nested)
            addHeadBlock(new MCLoopBlock(Sequential, Start, 0, 0, Insts, NULL, this->getMode(), this->getEd()));
          else
            for (unsigned i = 0; i < Blocks.size(); i++)
...
void MCLoopBlock::ExpandEmbeddedHM(StringMap<HMacro*> &Map) {
  ...
  if (Blocks.size() == 0 && Insts.size() != 0) {
    addSubBlock(new MCLoopBlock(Sequential, Start, 0, 0, this->getMode(), this->getEd()));
    delete Blocks.back();
    Blocks.pop_back();
  }
  ...
  while (!EmbeddedHMs.empty()) {
    ...
    if (start > Start) {
      ...
      if (start - Start == 1) {
        RHS.push_back(new MCLoopBlock(Sequential, Start, 0, 0, this->getMode(), this->getEd()));
        RHS.back()->addParsedInst(new MCParsedInst(Nops));
      } else {
        RHS.push_back(new MCLoopBlock(Repeat, Start, start - Start, 0, this->getMode(), this->getEd()));
        RHS.back()->addParsedInst(new MCParsedInst(Nops));
      }
    }
...
void MCLoopBlock::MergeBlock(std::vector<MCLoopBlock *> &LHS,
  ...
  while (!LHS.empty() && !RHS.empty()) {
    ...
    if (ltype == Sequential) {
      ...
      Blocks.push_back(new MCLoopBlock(Sequential, lindex, 0, 0, this->getMode(), this->getEd()));
      ...
      if (rtype == Loop &&
        ...
        if (LHS.front()->getCount() == RHS.front()->getCount()) {
          ...
          if (!LHS.front()->isNested()) {
            LHS.front()->addSubBlock(new MCLoopBlock(Sequential, 0, 0, 0, this->getMode(), this->getEd()));
            delete LHS.front()->getBlocks().back();
            LHS.front()->getBlocks().pop_back();
          }
          if (!RHS.front()->isNested()) {
            RHS.front()->addSubBlock(new MCLoopBlock(Sequential, 0, 0, 0, this->getMode(), this->getEd()));
...
void MCFunction::addParsedInst(MCParsedInst *Inst) {
  ...
  int64_t count = 1;
  unsigned ki = 0;
  unsigned mode = 0;//yangl op:mode is a reg
  int ed = 0;//yangl ed is imm
  bool Sym;

  if (!CurBlock) CurBlock = addLoopBlock(new MCLoopBlock(Sequential, Index, 0, 0, mode, ed));
    ...
    for (Sym = PendingLabels.back().second;
      ...
      if (CurBlock->size() == 0) {
        ...
        if (CurBlock->getParent())
          CurBlock = CurBlock->addSiblingBlock(new MCLoopBlock(Sequential, Index, 0, 0, mode, ed)); // this loop is also a nested one
        else
          CurBlock = addLoopBlock(new MCLoopBlock(Sequential, Index, 0, 0, mode, ed)); // this loop is at the top level
      }
    }
  }
  if (cachedLabel) {
    ...
    while (PreviousInst && PreviousInst->hasLoop(OpNo)) {
      op = CAST_TO_MMPU_OPRD(PreviousInst->getOperand(OpNo + 2));//fixed for UCPM::LPTO
      ...
      if (op->isExpr()) {
        ...
        if (CE) return; // absolute address is ignored
          ...
          if (SE) {
            ...
            op = CAST_TO_MMPU_OPRD(PreviousInst->getOperand(OpNo + 3));//fixed for UCPM::LPTO
            ki = op->getImm();
            op = CAST_TO_MMPU_OPRD(PreviousInst->getOperand(OpNo + 4));//fixed for UCPM::LPTO
            count = -op->getImm();
            op = CAST_TO_MMPU_OPRD(PreviousInst->getOperand(OpNo + 1));
            mode = op->getReg();
            op = CAST_TO_MMPU_OPRD(PreviousInst->getOperand(OpNo + 5));
            ed = op->getImm();
          } else return; // unknown loop label
        }
      }
      MCLoopBlock *SubLoop = new MCLoopBlock(Loop, Index, count, ki, mode, ed);
      //CurBlock->passEmbeddedHM(SubLoop);
      CurBlock = CurBlock->addSubBlock(SubLoop);
      PreviousInst->delOperand(OpNo); //delete LPTO
      PreviousInst->delOperand(OpNo); //delete mode
      PreviousInst->delOperand(OpNo); //delete Label
      PreviousInst->delOperand(OpNo); //delete KI
      PreviousInst->delOperand(OpNo); //delete KI difference(imm)
      PreviousInst->delOperand(OpNo); //delete ed
      ...
      while (PreviousInst->hasEmbeddedHMacro(OpNoH)) {
        ...
        Inst->delOperand(OpNoH);//fixme:not sure if it need to fixed for ucpm
      }
    }
  }
  if (Inst->hasRepeat(OpNo)) {//here is different from MaPU1.0 111
    op = CAST_TO_MMPU_OPRD(Inst->getOperand(OpNo));
    if (op->getOpc() == UCPM::REPEATIMM) {
      op = CAST_TO_MMPU_OPRD(Inst->getOperand(OpNo + 2));//attention: +2 is corresponding to the structure of UCPM::REPEATIMM
      count = op->getImm();
    ...
    if (op->getOpc() == UCPM::REPEATIMM && count == 0) count = 1024;
      ...
      ki = count;
      op = CAST_TO_MMPU_OPRD(Inst->getOperand(OpNo + 3));//fixed for UCPM::REPEATK
      count = -op->getImm();
      op = CAST_TO_MMPU_OPRD(Inst->getOperand(OpNo + 1));
      mode = op->getReg();
      op = CAST_TO_MMPU_OPRD(Inst->getOperand(OpNo + 4));
      ed = op->getImm();
    }
    if (count != 1) {
      if (CurBlock->size() != 0) {
        if (CurBlock->getType() == Loop)
          CurBlock = CurBlock->addSubBlock(new MCLoopBlock(Repeat, Index, count, ki, mode, ed));
        else if (CurBlock->getParent())
          CurBlock = CurBlock->addSiblingBlock(new MCLoopBlock(Repeat, Index, count, ki, mode, ed));
        else
          CurBlock = addLoopBlock(new MCLoopBlock(Repeat, Index, count, ki, mode, ed));
      }
      else {
        if (CurBlock->getType() == Loop)
          CurBlock = CurBlock->addSubBlock(new MCLoopBlock(Repeat, Index, count, ki, mode, ed));
    ...
    op = CAST_TO_MMPU_OPRD(Inst->getOperand(OpNo + 2)); //fixed for UCPM::LPTO
    //if (op->isImm()) return;  //absolute address is ignored in HMacro
    if (op->isExpr()) {
      const MCConstantExpr *CE = dyn_cast<MCConstantExpr>(op->getExpr());
      if (CE) return; //ignored
        ...
        if (SE) {
          ...
          op = CAST_TO_MMPU_OPRD(Inst->getOperand(OpNo + 4));
          count = -op->getImm();
          op = CAST_TO_MMPU_OPRD(Inst->getOperand(OpNo + 5));
          ed = op->getImm();
        }
      }
    }
  ...
  ++Index;
  if (Inst->hasRepeat(OpNo)) {//fixed for UCPM
    Inst->delOperand(OpNo); //delete Repeat
    Inst->delOperand(OpNo); //delete op: mode
    Inst->delOperand(OpNo); //delete op: imm
    Inst->delOperand(OpNo); //delete op: ed
    if (count <= 0) Inst->delOperand(OpNo); //delete KI difference
    if (count != 1) {
      if (CurBlock->getParent())
        CurBlock = CurBlock->addSiblingBlock(new MCLoopBlock(Sequential, Index, 0, 0, mode, ed));
      else
        CurBlock = addLoopBlock(new MCLoopBlock(Sequential, Index, 0, 0, mode, ed));
    }
  }
  else if (Inst->hasLoop(OpNo)) {
    if (CurBlock->getType() == Loop)
      CurBlock = CurBlock->addSubBlock(new MCLoopBlock(Loop, Index, count, ki, mode, ed));
    else if (CurBlock->getParent())
      CurBlock = CurBlock->addSiblingBlock(new MCLoopBlock(Loop, Index, count, ki, mode, ed));
    else
      CurBlock = addLoopBlock(new MCLoopBlock(Loop, Index, count, ki, mode, ed));
    Inst->delOperand(OpNo); //delete LPTO
    Inst->delOperand(OpNo); //delete mode
    Inst->delOperand(OpNo); //delete Label
    Inst->delOperand(OpNo); //delete KI
    Inst->delOperand(OpNo); //delete KI difference(imm)
    Inst->delOperand(OpNo); //delete ed
    ...
    while (Inst->hasEmbeddedHMacro(OpNoH)) {
      ...
      CurBlock->addEmbeddedHM(new MCFunction(*op->getHMacro(), Index - 1));
      Inst->delOperand(OpNoH);
      Inst->delOperand(OpNoH);//fixme: not sure if it need to be fixed for ucpm
    }
  }
  PreviousInst = Inst;
}
