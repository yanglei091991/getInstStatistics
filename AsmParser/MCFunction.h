...
public:
  MCLoopBlock(MCBlockTy T, uint64_t A = 0, int64_t C = 0, unsigned _KI = 0, unsigned _mode = 0, int _ed = 0)
    : Type(T), Start(A), Count(C), KI(_KI), mode(_mode), ed(_ed),
  ...
  MCLoopBlock(MCBlockTy T, uint64_t A, int64_t C, unsigned _KI,
              ...
              MCLoopBlock *Parent = NULL, unsigned _mode = 0, int _ed = 0)
    : Type(T), Start(A), Count(C), KI(_KI), Insts(I), mode(_mode), ed(_ed),
  ...
  MCLoopBlock(MCBlockTy T, uint64_t A, int64_t C, unsigned _KI,
              ...
              bool isKI, MCLoopBlock *Parent = NULL, unsigned _mode = 0, int _ed = 0)
    : Type(T), Start(A), Count(C), KI(_KI), Blocks(MB), mode(_mode), ed(_ed),
  ...
  MCLoopBlock *addSubBlock(MCLoopBlock* MB) {
    if (!Nested) {
      Nested = true;
      if (Insts.size())
        Blocks.push_back(new MCLoopBlock(Sequential, Start, 0, 0, Insts, this, mode, ed));
    }
    if (MB->getType() == Loop ||
        (MB->getType() == Repeat &&
         MB->getCount() <= 0))
